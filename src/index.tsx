...
const Toast = (props: ToastProps) => {
	...
	return (
		<li
			...
			data-sonner-toast=""
			data-styled={
				!Boolean(props.toast.jsx || props.toast.unstyled || props.unstyled)
			}
			...
			onPointerUp={(event) => {
				...
				const swipeAmount = Number(
					(event.target as HTMLLIElement)?.style
				...
				(event.target as HTMLLIElement)?.style.setProperty(
					"--swipe-amount",
					"0px",
				);
				setSwiping(false);
		...
		>
			...
			<Show
				when={
					!(
						props.toast.jsx != undefined ||
						(typeof props.toast.title !== "string" &&
							props.toast.title != undefined)
					)
				}
				fallback={props.toast.jsx || props.toast.title}
			>
				<>
					<Show
						when={
								...
								>
									{(toast, index) => (
										<Toast
											...
											invert={props.invert}
											visibleToasts={
												props.visibleToasts ?? VISIBLE_TOASTS_AMOUNT
											}
